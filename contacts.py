...
class MainTab():
    ...
    def __init__(self, root):
        ...
        ttk.Button(self.statFrame, text="Экспорт непос.", image=self.root.img[13], compound="left", command=self.exportNonVisit).pack(padx=self.root.padx, side="right", anchor="e")
    ...
    def newSave(self):
        ...
        self.root.save()
        self.root.log("Создан новый контакт в участке %s (%s, %s, %s)." % (self.selected.number, self.addressNew.get().strip(), self.nameNew.get().strip(), self.nonVisitNew.get().strip()))
        self.update()
    ...
    def getContent(self):        
        ...
        #elif self.sortCon.get()==1: self.content.sort(key=lambda x: x[4].address) 
        ...
        for i in range(len(self.content)):
            self.contentFormatted.append("%4d) %5s/%-25s %-25s %-25s %s" % (i+1, "№"+self.content[i][0][:5], self.content[i][4].address[:25], self.content[i][1][:25], self.content[i][2][:25], self.content[i][3]))
    ...
    def exportNonVisit(self):        
        ...
        row=0
        normal=xlwt.easyxf('alignment: shrink True')
        for i in range(len(self.content)):
            if self.content[i][3]!="":
                ws.write(row, 0, "№%s/%s" % (self.content[i][0], self.content[i][4].address), style=normal)
                ws.write(row, 1, self.content[i][1]+"\u00A0", style=normal)
                ws.write(row, 2, self.content[i][2]+"\u00A0", style=normal)
                row+=1                
        ws.col(0).width = 6000
        ws.col(1).width = 5000
        ws.col(2).width = 5000
        ...
        filename=filedialog.asksaveasfilename(filetypes=ftypes, initialfile='Контакты не для посещения.xls', defaultextension='.xls')
        if filename!="":
...
class TerTab():
    ...
    def export(self):
        ...
        remark =    xlwt.easyxf('alignment: shrink True;' 'font: height 250;' 'font: bold False;' 'alignment: horizontal center')
        ...
        header2=    xlwt.easyxf('alignment: shrink True;' 'font: height 250;' 'font: bold True;'  'alignment: horizontal center')
        ...
        ws.write_merge(0,0, 0,1, "Участок №%s (%s), контактов: %d" % (self.card.ter.number, self.card.ter.address, len(self.card.ter.extra[0])), style=header1)
        ws.write_merge(28,28, 0,1, "Последний обработал: %s %s" % (self.card.ter.getPublisherFinished(), self.card.ter.getDateLastSubmit()), style=header2)
        ws.col(0).width = 4000
        ws.col(1).width = 6000
        ...
        if len(self.card.ter.extra)!=0:                                         # writing contacts, if exist
            row=1
            col=0
            if len(self.card.ter.extra[0])>25: pagesTotal=2
            address=""            
            ...
            for e in self.card.ter.extra[0]:
                ...
                if e[2]!="": nonVisit=" (не пос. до %s)" % e[2]
                else: nonVisit="\u00A0"
                ...
                if row>=28:
                    ...
                    ws.col(col+1).width = 6000
                    row=1        
        ws.write_merge(29,29, 0,1, "(%s) Вернуть с участком! Стр. 1/%d" % (date, pagesTotal), style=remark)
        if pagesTotal==2:
            ws.write_merge(0,0, 2,3, "Участок №%s (%s), контактов: %d" % (self.card.ter.number, self.card.ter.address, len(self.card.ter.extra[0])), style=header1)
            ws.write_merge(29,29, 2,3, "(%s) Вернуть с участком! Стр. 2/2" % date, style=remark)
